...
{
	name = "Gelişmiş Discord Komut Log", 
	author = "ByDexter", 
	description = "", 
	version = "1.0", 
...
{
	AddCommandListener(CommandListener_Discord);
	LoopClientsValid(i)
	{
		OnClientPostAdminCheck(i);
	}
	devapikey = CreateConVar("sm_bydexter_apikey", "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX", "https://steamcommunity.com/dev/apikey Siteden api key alınız.", FCVAR_PROTECTED); devapikey.GetString(ApiKey, 33); devapikey.AddChangeHook(devapikeyget);
	webhook = CreateConVar("sm_discord-smlog_webhook", "", "Discord Webhook"); webhook.GetString(WebHook, 192); webhook.AddChangeHook(webhookget);
	AutoExecConfig(true, "discord-smlog", "ByDexter");
}
...
public Action CommandListener_Discord(int client, const char[] command, int argc)
{
	if (StrContains(command, "sm_", false) != -1 && IsValidClient(client))
	{
		char Arg[256];
		...
		Format(format, 256, "%s %s", command, Arg);
